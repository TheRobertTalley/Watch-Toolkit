...
uint32_t logo_timeout = 1000; // 1 second for EACH logo
...
constexpr char secretGestureSequence[] = {
    ...
    static_cast<char>(meshtastic_ModuleConfig_CannedMessageConfig_InputEventChar_RIGHT), // physical left
    static_cast<char>(meshtastic_ModuleConfig_CannedMessageConfig_InputEventChar_LEFT),  // physical right
    static_cast<char>(meshtastic_ModuleConfig_CannedMessageConfig_InputEventChar_LEFT),  // physical right
    static_cast<char>(meshtastic_ModuleConfig_CannedMessageConfig_InputEventChar_RIGHT), // physical left
    static_cast<char>(meshtastic_ModuleConfig_CannedMessageConfig_InputEventChar_DOWN),
    static_cast<char>(meshtastic_ModuleConfig_CannedMessageConfig_InputEventChar_UP)};
...
static void drawIconScreen(const char *upperMsg, OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x, int16_t y)
{
    ...
    display->drawXbm(x + (SCREEN_WIDTH - icon_width) / 2,
                     y + (SCREEN_HEIGHT - FONT_HEIGHT_MEDIUM - icon_height) / 2 + 2 + 19,
...
void Screen::drawDigitalClockFrame(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x, int16_t y)
{
    ...
    if (self && self->battMeterUnlocked) {
        ...
        drawBatteryMeterButton(display, display->getWidth() - 36, display->getHeight() - 36,
                               LowerRightButtonMode::BatteryPercent, batteryButtonPercent, 1);
    }
...
void Screen::drawBattMeterFrame(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x, int16_t y)
{
    ...
    for (uint8_t i = 0; i < percentString.length(); i++) {
    ...
    for (uint8_t i = 0; i < percentString.length(); i++) {
        String character = String(percentString[i]);
        uint8_t digit = character.toInt();
        drawSegmentedDisplayCharacter(display, drawX, drawY, digit, scale);
...
#if defined(T_WATCH_S3)
    // Render watch UI elements in bright red instead of the default teal
    static_cast<TFTDisplay *>(dispdev)->setMeshColor(COLOR565(0xFF, 0x00, 0x00));
#endif
...
void Screen::drawSecretMenuFrame(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x, int16_t y)
{
    ...
    // NOTE: Devices such as the T-Watch S3 mount the screen inverted relative to the buttons,
    // so the apparent on-screen directions are mirrored. Resist the urge to swap these prompts.
    const char *rootInstructionText = "Tap=Select  Left=Hide";
    const char *attackInstructionText = "Tap=Select  Left=Back";
    ...
    if (self->secretMenuMode == SecretMenuMode::WifiAttacks) {
        ...
        if (auto *tftDisplay = dynamic_cast<TFTDisplay *>(self->dispdev)) {
            ...
            tftDisplay->drawColorString(x + width / 2 - 40, y, "WiFi Attacks", secretMenuHeaderColor);

            int16_t listStartY = y + FONT_HEIGHT_MEDIUM + 8;
            int16_t lineHeight = FONT_HEIGHT_SMALL + 8;
            size_t firstIndex = 0;
            ...
            for (size_t i = 0; i < wifiAttackVisibleCount && (firstIndex + i) < wifiAttackItemCount; ++i) {
                ...
                if (itemIndex == self->wifiAttackSelection)
                    tftDisplay->drawColorString(x + 6, itemY, ">", secretMenuAccentColor);
                tftDisplay->drawColorString(x + 28, itemY, lineBuf, secretMenuTextColor);
            }

            tftDisplay->drawColorString(x + width / 2 - 70,
                                        y + height - (FONT_HEIGHT_SMALL + 2),
                                        attackInstructionText,
                                        secretMenuHeaderColor);
            return;
        }
        ...
        if (auto *tftDisplay = dynamic_cast<TFTDisplay *>(self->dispdev)) {
            ...
            int16_t height = display->getHeight();
            tftDisplay->fillRectColor(x, y, width, height, TFT_BLACK);
            tftDisplay->drawColorString(x + width / 2 - 40, y, "WiFi Scanner", secretMenuHeaderColor);
            ...
            if (wifiScanInProgress || wifiScanResults.empty()) {
                ...
                tftDisplay->drawColorString(x + 6, listStartY, status, secretMenuTextColor);
            } else {
            ...
            if (wifiPreferredValid) {
                ...
                snprintf(targetLine,
                         sizeof(targetLine),
                         "Target: %s",
                         wifiPreferredSsid.empty() ? "<hidden>" : wifiPreferredSsid.c_str());
                tftDisplay->drawColorString(x + 6,
                                            y + height - 2 * (FONT_HEIGHT_SMALL + 4),
                                            targetLine,
    ...
    if (self->secretMenuMode == SecretMenuMode::WifiAttacks) {
        ...
        display->setTextAlignment(TEXT_ALIGN_CENTER);
        display->setFont(FONT_MEDIUM);
        display->drawString(x + display->getWidth() / 2, y, "WiFi Attacks");

        display->setTextAlignment(TEXT_ALIGN_LEFT);
        display->setFont(FONT_SMALL);

        int16_t listStartY = y + FONT_HEIGHT_MEDIUM + 6;
        int16_t lineHeight = FONT_HEIGHT_SMALL + 6;
        size_t firstIndex = 0;
        if (wifiAttackItemCount > wifiAttackVisibleCount && self->wifiAttackSelection >= wifiAttackVisibleCount)
            firstIndex = self->wifiAttackSelection - (wifiAttackVisibleCount - 1);
        if (firstIndex + wifiAttackVisibleCount > wifiAttackItemCount)
        ...
        for (size_t i = 0; i < wifiAttackVisibleCount && (firstIndex + i) < wifiAttackItemCount; ++i) {
            ...
            snprintf(line,
                     sizeof(line),
                     "%c %s%s",
                     (itemIndex == self->wifiAttackSelection) ? '>' : ' ',
                     marauder::wifiAttackLabel(itemIndex),
                     (attackRunning && (itemIndex == runningIndex)) ? " *" : "");
            display->drawString(x + 6, itemY, line);
        }

        display->setTextAlignment(TEXT_ALIGN_CENTER);
        display->drawString(x + display->getWidth() / 2,
                            y + display->getHeight() - 2 * (FONT_HEIGHT_SMALL + 2),
                            targetLine.c_str());
        display->drawString(x + display->getWidth() / 2,
                            y + display->getHeight() - (FONT_HEIGHT_SMALL + 2),
                            attackInstructionText);
    ...
    if (self->secretMenuMode == SecretMenuMode::WifiScanner) {
        ...
        display->setTextAlignment(TEXT_ALIGN_CENTER);
        display->setFont(FONT_MEDIUM);
        display->drawString(x + display->getWidth() / 2, y, "WiFi Scanner");
        ...
        if (wifiScanInProgress || wifiScanResults.empty()) {
            const char *status = wifiScanInProgress ? "Scanning..." : wifiScanStatus.c_str();
            display->drawString(x + 6, listStartY, status);
        } else {
            size_t visible = wifiAttackVisibleCount;
            size_t firstIndex = 0;
            if (wifiScanResults.size() > visible && wifiScanSelection >= visible)
    ...
    if (self->secretMenuMode == SecretMenuMode::StationAps || self->secretMenuMode == SecretMenuMode::StationStations) {
#if HAS_WIFI && defined(ARCH_ESP32)
        auto &tracker = marauder::StationTracker::instance();
        const auto &apList = tracker.getAccessPoints();
        const auto &stationList = tracker.getStations();
        display->setColor(BLACK);
        display->fillRect(x, y, display->getWidth(), display->getHeight());
        display->setColor(WHITE);
        if (self->secretMenuMode == SecretMenuMode::StationAps) {
            display->setTextAlignment(TEXT_ALIGN_CENTER);
            display->setFont(FONT_MEDIUM);
            display->drawString(x + display->getWidth() / 2, y, "Station Browser");
            display->setTextAlignment(TEXT_ALIGN_LEFT);
            display->setFont(FONT_SMALL);
            int16_t listStartY = y + FONT_HEIGHT_MEDIUM + 6;
            int16_t lineHeight = FONT_HEIGHT_SMALL + 6;
            size_t visible = wifiAttackVisibleCount;
            size_t firstIndex = 0;
            if (apList.size() > visible && stationApSelection >= visible)
                firstIndex = stationApSelection - (visible - 1);
            if (firstIndex + visible > apList.size())
                firstIndex = (apList.size() > visible) ? apList.size() - visible : 0;
            if (apList.empty()) {
                display->drawString(x + 6, listStartY, "Listening for APs...");
            } else {
                for (size_t i = 0; i < visible && (firstIndex + i) < apList.size(); ++i) {
                    size_t itemIndex = firstIndex + i;
                    int16_t itemY = listStartY + lineHeight * i;
                    const auto &ap = apList[itemIndex];
                    char line[96];
                    snprintf(line,
                             sizeof(line),
                             "%c %s (%u)",
                             (itemIndex == stationApSelection) ? '>' : ' ',
                             ap.ssid.empty() ? macToString(ap.bssid).c_str() : ap.ssid.c_str(),
                             static_cast<unsigned>(ap.stationIndices.size()));
                    display->drawString(x + 6, itemY, line);
                }
            }
            display->setTextAlignment(TEXT_ALIGN_CENTER);
            ...
            display->drawString(x + display->getWidth() / 2,
                                y + display->getHeight() - (FONT_HEIGHT_SMALL + 2),
                                stationApInstructionText);
        } else {
            display->setTextAlignment(TEXT_ALIGN_CENTER);
            display->setFont(FONT_MEDIUM);
            display->drawString(x + display->getWidth() / 2, y, "Stations");
            display->setTextAlignment(TEXT_ALIGN_LEFT);
            display->setFont(FONT_SMALL);
            if (stationApSelection >= apList.size()) {
                display->drawString(x + 6, y + FONT_HEIGHT_SMALL + 2, "No AP selected");
            } else {
                const auto &ap = apList[stationApSelection];
                char title[96];
                snprintf(title,
                         sizeof(title),
                         "%s",
                         ap.ssid.empty() ? macToString(ap.bssid).c_str() : ap.ssid.c_str());
                display->drawString(x + 6, y + FONT_HEIGHT_SMALL + 2, title);
                int16_t listStartY = y + FONT_HEIGHT_MEDIUM + 8;
                int16_t lineHeight = FONT_HEIGHT_SMALL + 6;
                size_t visible = wifiAttackVisibleCount;
                ...
                size_t firstIndex = 0;
                if (total > visible && stationStaSelection >= visible)
                    firstIndex = stationStaSelection - (visible - 1);
                if (firstIndex + visible > total) {
                    firstIndex = (total > visible) ? total - visible : 0;
                }
                size_t selectedIdx = marauder::StationTracker::instance().selectedStationIndex();
                if (indices.empty()) {
                    display->drawString(x + 6, listStartY, "No stations detected");
                } else {
                    for (size_t i = 0; i < visible && (firstIndex + i) < total; ++i) {
                        size_t itemIndex = indices[firstIndex + i];
                        if (itemIndex >= stationList.size())
                            continue;
                        const auto &sta = stationList[itemIndex];
                        bool targeted = selectedIdx < stationList.size() && selectedIdx == itemIndex;
                        int16_t itemY = listStartY + lineHeight * i;
                        char line[96];
                        snprintf(line,
                                 sizeof(line),
                                 "%c %s (%ddBm)%s",
                                 ((firstIndex + i) == stationStaSelection) ? '>' : ' ',
                                 macToString(sta.mac).c_str(),
                                 static_cast<int>(sta.rssi),
                                 targeted ? " *" : "");
                        display->drawString(x + 6, itemY, line);
                    }
                }
    ...
    if (auto *tftDisplay = dynamic_cast<TFTDisplay *>(self->dispdev)) {
        ...
        tftDisplay->drawColorString(targetX,
                                    y + height - (FONT_HEIGHT_SMALL + 19),
                                    targetLine.c_str(),
                                    secretMenuHeaderColor);
        tftDisplay->drawColorString(x + width / 2 - 70, y + height - (FONT_HEIGHT_SMALL + 2), rootInstructionText,
                                    secretMenuHeaderColor);
            return;
        }
    } else if (self->secretMenuMode == SecretMenuMode::StationAps || self->secretMenuMode == SecretMenuMode::StationStations) {
#if HAS_WIFI && defined(ARCH_ESP32)
        if (auto *tftDisplay = dynamic_cast<TFTDisplay *>(self->dispdev)) {
            auto &tracker = marauder::StationTracker::instance();
            const auto &apList = tracker.getAccessPoints();
            ...
            tftDisplay->fillRectColor(x, y, width, height, TFT_BLACK);
            if (self->secretMenuMode == SecretMenuMode::StationAps) {
                tftDisplay->drawColorString(x + width / 2 - 40, y, "Station Browser", secretMenuHeaderColor);
                int16_t listStartY = y + FONT_HEIGHT_MEDIUM + 8;
                int16_t lineHeight = FONT_HEIGHT_SMALL + 8;
                size_t visible = wifiAttackVisibleCount;
                size_t firstIndex = 0;
                ...
                if (firstIndex + visible > apList.size())
                    firstIndex = (apList.size() > visible) ? apList.size() - visible : 0;
                if (apList.empty()) {
                    tftDisplay->drawColorString(x + 6, listStartY, "Listening for APs...", secretMenuTextColor);
                } else {
                    for (size_t i = 0; i < visible && (firstIndex + i) < apList.size(); ++i) {
                        size_t itemIndex = firstIndex + i;
                        int16_t itemY = listStartY + lineHeight * i;
                ...
                tftDisplay->drawColorString(x + width / 2 - 70,
                                            y + height - 2 * (FONT_HEIGHT_SMALL + 2),
                                            targetLine.c_str(),
                                            secretMenuHeaderColor);
                tftDisplay->drawColorString(x + width / 2 - 70,
                                            y + height - (FONT_HEIGHT_SMALL + 2),
                ...
                if (stationApSelection >= apList.size()) {
                    tftDisplay->drawColorString(x + 6, y + FONT_HEIGHT_SMALL + 4, "No AP selected", secretMenuTextColor);
                } else {
                    const auto &ap = apList[stationApSelection];
                    tftDisplay->drawColorString(x + width / 2 - 40, y, "Stations", secretMenuHeaderColor);
                    char title[96];
                    snprintf(title,
                             sizeof(title),
                             "%s",
                             ap.ssid.empty() ? macToString(ap.bssid).c_str() : ap.ssid.c_str());
                    tftDisplay->drawColorString(x + 6, y + FONT_HEIGHT_SMALL + 2, title, secretMenuAccentColor);
                    int16_t listStartY = y + FONT_HEIGHT_MEDIUM + 12;
                    int16_t lineHeight = FONT_HEIGHT_SMALL + 8;
                    size_t visible = wifiAttackVisibleCount;
                    const auto &indices = ap.stationIndices;
                    size_t total = indices.size();
                    ...
                    size_t selectedIdx = tracker.selectedStationIndex();
                    if (indices.empty()) {
                        tftDisplay->drawColorString(x + 6, listStartY, "No stations detected", secretMenuTextColor);
                    } else {
                        for (size_t i = 0; i < visible && (firstIndex + i) < total; ++i) {
                            size_t itemIndex = indices[firstIndex + i];
                            if (itemIndex >= stationList.size())
                            ...
                            snprintf(lineBuf,
                                     ...
                                     static_cast<int>(sta.rssi),
                                     targeted ? " *" : "");
                            if ((firstIndex + i) == stationStaSelection)
                                tftDisplay->drawColorString(x + 6, itemY, ">", secretMenuAccentColor);
                            tftDisplay->drawColorString(x + 28, itemY, lineBuf, secretMenuTextColor);
                        }
                    }
    ...
    int16_t bgX = x + (display->getWidth() - secretMenuEagleWidth) / 2;
    int16_t bgY = y + (display->getHeight() - secretMenuEagleHeight) / 2 - 30;
    if (bgX < x)
        bgX = x;
    if (bgY < y)
        bgY = y;
    if (secretMenuEagleWidth <= display->getWidth() && secretMenuEagleHeight <= display->getHeight()) {
        display->drawXbm(bgX, bgY, secretMenuEagleWidth, secretMenuEagleHeight, secretMenuEagleBits);
    }
    ...
    display->setTextAlignment(TEXT_ALIGN_CENTER);
    display->drawString(x + display->getWidth() / 2,
                        y + display->getHeight() - (FONT_HEIGHT_SMALL + 19),
                        targetLine.c_str());
    display->drawString(x + display->getWidth() / 2,
                        y + display->getHeight() - (FONT_HEIGHT_SMALL + 2),
                        rootInstructionText);
...
bool Screen::handleSecretMenuInput(char inputEvent)
{
    ...
    auto handleWifiAttackNav = [this](char inputEvent) -> bool {
        switch (inputEvent) {
    ...
    auto handleWifiScanNav = [this](char inputEvent) -> bool {
        switch (inputEvent) {
        case static_cast<char>(meshtastic_ModuleConfig_CannedMessageConfig_InputEventChar_UP):
    ...
    auto handleStationApNav = [this](char inputEvent) -> bool {
#if HAS_WIFI && defined(ARCH_ESP32)
        auto &tracker = marauder::StationTracker::instance();
    ...
    auto handleStationStaNav = [this](char inputEvent) -> bool {
#if HAS_WIFI && defined(ARCH_ESP32)
        auto &tracker = marauder::StationTracker::instance();
        const auto &aps = tracker.getAccessPoints();
    ...
    case static_cast<char>(meshtastic_ModuleConfig_CannedMessageConfig_InputEventChar_CANCEL):
        ...
        return true;
    default:
        return true; // swallow other inputs while menu open
    }
}

void Screen::updateSecretGestureProgress(char inputEvent)
{
    auto isDirectional = [](char evt) {
        return evt == static_cast<char>(meshtastic_ModuleConfig_CannedMessageConfig_InputEventChar_UP) ||
               evt == static_cast<char>(meshtastic_ModuleConfig_CannedMessageConfig_InputEventChar_DOWN) ||
               evt == static_cast<char>(meshtastic_ModuleConfig_CannedMessageConfig_InputEventChar_LEFT) ||
               evt == static_cast<char>(meshtastic_ModuleConfig_CannedMessageConfig_InputEventChar_RIGHT);
    };
...
int Screen::handleInputEvent(const InputEvent *event)
{
    ...
    if (touchedBatteryButton) {
        if (this->ui->getUiState()->currentFrame == watchFaceFrame) {
            if (battMeterUnlocked && !battMeterActive) {
                startBattMeterMode();
            }
