...
class Screen : public concurrency::OSThread
{
  ...
  private:
    ...
    void updateSecretGestureProgress(char inputEvent);
    bool handleSecretMenuInput(char inputEvent);
      void showSecretMenu();
      void hideSecretMenu();
      void handleSecretMenuSelection();
      void startWiFiSnifferTool();
      void startWiFiAttackTool(size_t attackIndex);
      void startWifiScanList();
      ...
      uint8_t secretGestureProgress = 0;
      enum class SecretMenuMode
      {
          Root,
          WifiAttacks,
          WifiScanner,
