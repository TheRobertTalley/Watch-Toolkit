...
uint16_t TFTDisplay::meshColor = TFT_MESH;
        ...
        for (x = 0; x < displayWidth; x++) {
            ...
            if (!fromBlank) {
                ...
                if (isset != dblbuf_isset) {
                    tft->drawPixel(x, y, isset ? meshColor : TFT_BLACK);
                }
            } else if (isset) {
                tft->drawPixel(x, y, meshColor);
            }
        }
    }
...
void TFTDisplay::setMeshColor(uint16_t color)
{
    meshColor = color;
}

#if HAS_TFT
void TFTDisplay::drawColorString(int16_t x, int16_t y, const String &text, uint16_t color, uint16_t bg)
{
    if (!tft)
        return;
...
void TFTDisplay::fillRectColor(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color)
{
    if (tft)
        tft->fillRect(x, y, w, h, color);
}

void TFTDisplay::drawXBitmapColor(int16_t x, int16_t y, const uint8_t *bitmap, int16_t w, int16_t h, uint16_t color)
{
    if (tft)
