...
#else
                TFT_MESH = COLOR565(0xFF, 0x00, 0x00);
#endif
            } else {
...
void menuHandler::testMenu()
{
    ...
    enum optionsNumbers { Back, SecretTools, NumberPicker, ShowChirpy, enumEnd };
    static const char *optionsArray[enumEnd] = {"Back"};
    static int optionsEnumArray[enumEnd] = {Back};
    int options = 1;

    optionsArray[options] = "Secret Tools";
    optionsEnumArray[options++] = SecretTools;
    ...
    bannerOptions.optionsEnumPtr = optionsEnumArray;
    bannerOptions.bannerCallback = [](int selected) -> void {
        if (selected == SecretTools) {
            menuQueue = secret_tools_menu;
            screen->runNow();
        } else if (selected == NumberPicker) {
...
void menuHandler::handleMenuSwitch(OLEDDisplay *display)
{
    ...
    case test_menu:
        testMenu();
        break;
    case secret_tools_menu:
        screen->showSecretToolsMenu();
        break;
    case number_test:
        numberTest();
        break;
