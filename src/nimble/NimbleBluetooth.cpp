...
static void handlePhoneNotification(const NotifService::NotificationPayload &payload);
...
void NimbleBluetooth::setupService()
{
    ...
    NotifService::instance().begin(bleServer, handlePhoneNotification);
}
...
static void handlePhoneNotification(const NotifService::NotificationPayload &payload)
{
    ...
    if (notificationManager) {
        if (payload.clear) {
            ...
            std::string title = !payload.title.empty() ? payload.title
                                                       : (!payload.app.empty() ? payload.app : "Notification");
            std::string body = !payload.body.empty() ? payload.body
                                                     : (!payload.title.empty() ? payload.title : payload.app);
